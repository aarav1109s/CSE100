# Same checks as ./Grade (bash). Run from Lab10 after building a.exe.
$ErrorActionPreference = "Stop"
Set-Location $PSScriptRoot

if (-not (Test-Path ".\a.exe")) {
    Write-Host "Build first: g++ -std=c++11 -O2 -o a.exe aaravsharma.cpp"
    exit 1
}

if (-not (Test-Path ".\testfiles\t1")) {
    Write-Host "Missing testfiles (t1..t5, o1..o5)."
    exit 1
}
